...
public class TfIdf {
	...
	public double TFIDFtesting(Set<String> terms, HashMap<String, Double> weight) {
		// The Ranking function
		// relevence(d) = sumation of all terms(w(term) if term in d);
		double rank = 0.0;
		Set<String> documentTerms = weight.keySet();
		for (String term : terms) {
			if (documentTerms.contains(term)) {
				rank = rank + weight.get(term);
			}
		}
		return rank;
	}
